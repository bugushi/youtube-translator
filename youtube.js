(function () {
    ...
    // prevent not register success for the loading lantency
    let count = 0;
    let intervalId = setInterval(() => {
        if (count > 50 || videoElement) {
            clearInterval(intervalId);
            return;
        }
        videoElement = document.querySelector('video');
        videoElement && registerListeners(videoElement);
        count++;
    }, 200);

    function registerListeners(videoElement) {
